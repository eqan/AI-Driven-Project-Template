...
class StatsService:
    ...
    async def generate_stats(self, user_id: int):
        ...
        try:
            ...
            with session_scope() as session:
                existing_stats = session.query(ConversationStats).filter(ConversationStats.user_id == user_id).first()
                if existing_stats:
    ...
    async def get_stats(self, user_id: int):
        try:
            with session_scope() as session:
                stats = session.query(ConversationStats).filter(ConversationStats.user_id == user_id).first()
                return stats
        except SQLAlchemyError as e:
            raise HTTPException(status_code=500, detail=str(e))
